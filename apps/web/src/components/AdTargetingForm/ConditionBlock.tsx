...
export function ConditionBlock({ condition, onUpdate, onRemove, platform }: ConditionBlockProps) {
  ...
  // Get contextual label for the value field
  const getValueLabel = () => {
    if (!schema) return 'Value';
    
    // Check for specific attribute patterns
    if (schema.attribute.includes('percentage') || schema.attribute.includes('percent')) {
      return 'Percentage';
    }
    if (schema.attribute.includes('count') || schema.attribute.includes('followers')) {
      return 'Count';
    }
    if (schema.attribute.includes('age')) {
      return 'Age';
    }
    if (schema.attribute.includes('score')) {
      return 'Score';
    }
    if (schema.attribute.includes('views')) {
      return 'Views';
    }
    if (schema.attribute.includes('likes')) {
      return 'Likes';
    }
    if (schema.attribute.includes('time')) {
      return 'Time';
    }
    
    // Default based on type
    if (schema.type === 'number') return 'Number';
    if (schema.type === 'boolean') return 'Status';
  ...
  const renderValueInput = () => {
    ...
    // Skip value input if schema has params that define the value
    // (e.g., audience_active_time uses time_range param instead of value)
    if (schema.params && schema.params.length > 0 && 
        (schema.attribute.includes('active_time') || 
         schema.operators.includes('during'))) {
      return null;
    }
    ...
    const isPercentage = schema.attribute.includes('percentage') || schema.attribute.includes('percent');
        ...
        <>
          ...
          {schema.operators.length > 1 ? (
            <div className="grid grid-cols-2 gap-3">
              ...
              {renderValueInput() && (
                <div className="space-y-2">
                  <Label className="text-base font-medium">{getValueLabel()}</Label>
                  {renderValueInput()}
                </div>
              )}
            </div>
